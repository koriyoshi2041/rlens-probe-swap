#!/usr/bin/env python
"""Block 31: is the two-dimensional lens subspace the bottleneck for the items that never flip?
...
def main() -> int:
    out_dir = results_dir("block31_donor_paste")
    model = load_model()
    lenses = load_lenses()
    lens = lenses["J"]
    items = load_items()
    ...
    if foil_mode:
        out_dir = results_dir("block31_donor_paste_foil")
