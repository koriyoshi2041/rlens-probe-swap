#!/usr/bin/env python
"""Block 32: the block-30 answer-emergence decomposition on Qwen3.5-4B (second model).
...
OUT_NAME = f"block32_4b_answer_emergence_band{_lo}_{_hi}"
...
def main() -> int:
    ...
    for n, index in enumerate(eligible):
        ...
        for kind, lens in lenses.items():
            ...
            store = {}
            hooks = capturing_clamp_hooks(model, bases, coords, BAND, store)
