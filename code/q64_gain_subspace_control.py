#!/usr/bin/env python
"""Block 59: block-29 gain controls plus an in-subspace random field (audit M-1 control).
...
def main() -> int:
    out_dir = results_dir("block59_gain_controls_subspace")
    model = load_model()
    lenses = load_lenses()
    items = load_items()
    ...
    for layer in BAND:
        ...
        _, _, vh = torch.linalg.svd(j, full_matrices=False); svd_top[layer] = vh[:256]
    ...
    for n, index in enumerate(eligible):
        ...
        with torch.inference_mode():
            ...
            for kind, lens in lenses.items():
                ...
                # audit M-1 control: random field restricted to J_l's top-256 right-singular subspace, same norm profile
                rt = {}
                for layer in BAND:
                    vh = svd_top[layer].to(clamp_field[layer].device)
                    g = torch.randn(clamp_field[layer].shape[0], 256, generator=gen).to(clamp_field[layer].device)
                    rt[layer] = g @ vh
                fields["random_top256"] = match_norms(rt, clamp_field)
